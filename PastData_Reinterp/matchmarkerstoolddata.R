##Let's apply our single cell RNAseq markers to three past datasets##
...
HC=read.csv("HSC_markers.csv")
HC_markers = HC[HC$Hematopoietic.Cells.P.Value < .05,]
HC_markers$type = "HC"
HC_markers=HC_markers[,c(1:2,6)]

Neut = read.csv("Neut_markers.csv")
Neut_markers = Neut[Neut$Neutrophils.P.Value < .05,]
Neut_markers$type = "Neutrophil"
Neut_markers=Neut_markers[,c(1:2,6)]

APC = read.csv("APC_markers.csv")
APC_markers = APC[APC$Antigen.Presenting.Myeloid.Cells.P.Value < .05,]
APC_markers$type = "APC"
APC_markers=APC_markers[,c(1:2,6)]

Bcell = read.csv("Bcell_markers.csv")
Bcell_markers = Bcell[Bcell$B.Cells.P.Value < 0.05, ]
Bcell_markers$type = "Bcell"
Bcell_markers=Bcell_markers[,c(1:2,6)]

RBC = read.csv("RBC_markers.csv")
RBC_markers = RBC[RBC$Erythrocytes.P.Value < 0.05, ]
RBC_markers$type = "RBC"
RBC_markers=RBC_markers[,c(1:2,6)]

Plat = read.csv("Plat_markers.csv")
Plat_markers = Plat[Plat$Platelets.P.Value < 0.05, ]
Plat_markers$type = "Platelet"
Plat_markers=Plat_markers[,c(1:2,6)]

Fib = read.csv("Fib_markers.csv")
Fib_markers = Fib[Fib$Fibroblasts.P.Value < 0.05, ]
Fib_markers$type = "Fibroblast"
Fib_markers=Fib_markers[,c(1:2,6)]

NKC = read.csv("NKC_markers.csv")
NKC_markers = NKC[NKC$Natural.Killer.Cells.P.Value < 0.05, ]
